...
import pers.qingyu.sachima.sql.ast.statement.SQLCreateTriggerStatement.TriggerType;
import pers.qingyu.sachima.sql.dialect.mysql.ast.FullTextType;
import pers.qingyu.sachima.sql.dialect.mysql.ast.MySqlKey;
import pers.qingyu.sachima.sql.dialect.mysql.ast.statement.*;
import pers.qingyu.sachima.sql.dialect.mysql.parser.MySqlExprParser;
import pers.qingyu.sachima.sql.dialect.oracle.parser.OracleExprParser;
import pers.qingyu.sachima.sql.repository.SchemaRepository;
...
import static pers.qingyu.sachima.sql.parser.Token.ALL;
import static pers.qingyu.sachima.sql.parser.Token.ALTER;
import static pers.qingyu.sachima.sql.parser.Token.CASCADE;
import static pers.qingyu.sachima.sql.parser.Token.COMMA;
import static pers.qingyu.sachima.sql.parser.Token.GROUP;
import static pers.qingyu.sachima.sql.parser.Token.LINE_COMMENT;
import static pers.qingyu.sachima.sql.parser.Token.LITERAL_CHARS;
import static pers.qingyu.sachima.sql.parser.Token.LPAREN;
import static pers.qingyu.sachima.sql.parser.Token.MULTI_LINE_COMMENT;
import static pers.qingyu.sachima.sql.parser.Token.RESTRICT;
...
//                        statementList.add(stmt);
        ...
        if (lexer.token == Token.TABLE) {
            ...
            for (;;) {
                if (lexer.token == Token.DROP) {
                    ...
                    } else if (lexer.identifierEquals(FnvHash.Constants.LOCATION)) {
                        lexer.nextToken();
    ...
    public SQLStatement parseList() {
        if (lexer.identifierEquals(FnvHash.Constants.LIST)) {
            ...
            if (lexer.identifierEquals(FnvHash.Constants.RESOURCE)) {
                this.lexer.nextToken();
                if (lexer.identifierEquals(FnvHash.Constants.GROUP) || lexer.token == GROUP) {
                    this.lexer.nextToken();
    ...
    protected SQLDropTableStatement parseDropTable(boolean acceptDrop) {
        ...
        if (lexer.token == TABLE) {
            lexer.nextToken();
        } else if (lexer.identifierEquals(FnvHash.Constants.TABLES) && dbType == DbType.mysql) {
            lexer.nextToken();
        } else {
    ...
    protected SQLAlterTableAddIndex parseAlterTableAddIndex() {
        ...
        } else if (lexer.identifierEquals(FnvHash.SqlDataTypeConstants.SPATIAL)) {
            lexer.nextToken();
            item.setType("SPATIAL");
        } else if (lexer.identifierEquals(FnvHash.SqlDataTypeConstants.CLUSTERED)) {
            lexer.nextToken();
            item.setType("CLUSTERED");
        } else if (lexer.identifierEquals(FnvHash.SqlDataTypeConstants.ANN)) {
            lexer.nextToken();
            item.setType("ANN");
        }

        if (lexer.identifierEquals(FnvHash.SqlDataTypeConstants.GLOBAL)) {
            item.setGlobal(true);
            lexer.nextToken();
        }
        ...
        if (dbType == DbType.mysql) {
            for (; ; ) {
                if (lexer.identifierEquals(FnvHash.SqlDataTypeConstants.DISTANCEMEASURE)
                        || lexer.identifierEquals(FnvHash.SqlDataTypeConstants.ALGORITHM)
        ...
        if (DbType.mysql == dbType) {
            ...
            if (lexer.identifierEquals(FnvHash.SqlDataTypeConstants.DBPARTITION)) {
                SQLPartitionBy partitionClause = this.getSQLCreateTableParser().parsePartitionBy();
                item.setDbPartitionBy(partitionClause);
            }

            if (lexer.identifierEquals(FnvHash.SqlDataTypeConstants.TBPARTITION)) {
                lexer.nextToken();
                accept(Token.BY);
                SQLExpr expr = this.exprParser.expr();
                if (lexer.identifierEquals(FnvHash.SqlDataTypeConstants.STARTWITH)) {
                    lexer.nextToken();
            ...
            if (lexer.identifierEquals(FnvHash.SqlDataTypeConstants.TBPARTITIONS)) {
                lexer.nextToken();
        ...
        for (;;) {
            if (lexer.token == Token.COMMENT) {
                ...
                if (lexer.identifierEquals(FnvHash.SqlDataTypeConstants.KEY_BLOCK_SIZE)) {
                    lexer.nextToken();
                    if (lexer.token() == Token.EQ) {
                        lexer.nextToken();
                    }
